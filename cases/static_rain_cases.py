...
# from cases.static_cases import Static_BaseCase
...
class Static_BaseCase(ABC):
    def __init__(self):
        ...
        self.loss_weights =np.array([10.,10.,10.,1,1,1,]+            #pde loss weights
...
# top boundary
        ...
        # [loss(pde),loss(bcs),loss(ics)] 
        self.loss_weights=np.array([10.,1.,1.,1,10,10]+            #pde loss weights
                                                    [1,1,  1,1]+            #bcs loss weights
                                                    [1,1,1,1]+              #bcs loss weights     
                                                    [100,100,100])                #ics loss weights    
    ...
    def rain_intensity(self,points):
        ...
        rain=73.77884395200002*(_temp*0.13+18.1)/torch.pow(_temp+18.1,1.870) / 10. #mm/min-->cm/min
...
class Static_Cosine_Bump_Rain_Case(Static_Rain_BaseCase):
    ...
    def ic_func_of_h(self,points):   
        ...
        h[index]=self.height*100/2.*(1.+np.cos(np.pi/self.radius**2*x_square_add_y_square[index]))   # inside the unit circle
        return self.water_level-h                                        # finally 1-h is the initial condition of h  
...
class Static_Step_Rain_Case(Static_Rain_BaseCase):
    ...
    def ic_func_of_h(self,points):   
        ...
        z[ring_index]=self.height*10/self.a*(1.+self.a-r_ring_index)
        return self.water_level-z                                       
    ...
    def z_func(self,points)->tuple[torch.tensor,torch.tensor,torch.tensor]:
        ...
        z[ring_index]=self.height/self.a*(1.+self.a-r_ring_index)   
...
class Static_Tidal_Rain_Case(Static_Rain_BaseCase):
    ...
    def ic_func_of_h(self,points):   
        ...
        """     
        x,y=points[:,0:1],points[:,1:2]
        z_cm=self.height*100*np.cos(np.pi/self.geom[0][1]*x)*np.cos(np.pi/self.geom[1][1]*y)
...
# tidal=Static_Tidal_Rain_Case()
# tidal.plot_topograph()
# # tidal.plot_initial_condition()