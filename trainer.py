...
class HookedStdout:
    """A class to redirect stdout to a file and stdout."""
    ...
    def write(self, data):
        self.stdout.write(data)  # write to stdout
        self.file.write(data)    # write to file

    def flush(self):
        self.stdout.flush()      # flush stdout
...
class Trainer:
    ...
    def add_task(self, get_model, train_args):
        data = dill.dumps((get_model, train_args))   # serialize the model and training arguments
        self.tasks.append((data, train_args))        # add the serialized data and training arguments to the task list
    ...
    def train_all(self):
        ...
        for j in range(self.repeat):
            for i, (data, _) in enumerate(self.tasks):
                ...
                os.makedirs(save_path, exist_ok=True) # create a new directory for each task

                # 将输出信息输出到控制台和指定的文件中。
                hooked = HookedStdout(f"{save_path}/log.txt")
                sys.stdout = hooked
                #该实例同时将错误信息输出到控制台和指定的文件中。
                sys.stderr = HookedStdout(f"{save_path}/logerr.txt", sys.stderr)
                dde.config.set_random_seed(seed)

                print(f"***** Begin #{i}-{j} *****")
                get_model, train_args = dill.loads(data) # deserialize the model and training arguments
                model = get_model()
                model.train(**train_args, model_save_path=save_path)
                print(f"*****  End #{i}-{j}  *****")
